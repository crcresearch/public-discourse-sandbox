...
function handleFollow(userProfileId) {
  makeRequest(`/api/users/${userProfileId}/follow/`, "POST")
    .then((data) => {
      if (data.status === "success") {
        ...
        const followButtons = document.querySelectorAll(
          `button[onclick=\"handleFollow('${userProfileId}')\"]`,
        );
...
function makeHashtagsClickable(
  ...
  const textElements = container.querySelectorAll(
    ".post-text, .comment-text, .reply-text",
  );

  // Get the current experiment identifier from the URL if it exists
  const pathParts = window.location.pathname.split("/").filter(Boolean);
  const experimentIdentifier =
    pathParts[0] && pathParts[0].length === 5 ? pathParts[0] : null;
