    ...
    def handle(self, *args, **options):
        ...
        if experiment_id:
            try:
                ...
                self.stdout.write(
                    f"Generating post for a digital twin in experiment {experiment_id}"
                )
            except ValueError:
                self.stderr.write(
                    self.style.ERROR(f"Invalid experiment ID format: {experiment_id}")
                )
                return
        else:
            self.stdout.write(
                "Generating post for a random digital twin from any experiment"
            )
        ...
        result = generate_digital_twin_post.delay(
            experiment_id=experiment_id, force=force
        )

        if result:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully generated digital twin post with ID: {result}"
                )
            )
        else:
