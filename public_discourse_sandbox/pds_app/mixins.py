...
class ModeratorPermissionMixin:
    ...
    def is_moderator(self, user, experiment):
        ...
        return (
            experiment.creator == user  # User owns the experiment
            or user_profile.is_collaborator  # User is a collaborator
            or user_profile.is_moderator  # User has moderator flag
        )
...
class ProfileRequiredMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        needs_profile = (
            ...
            (self.experiment.creator == request.user and not self.user_profile)
            or
            # Invited user without profile
            (
                not self.user_profile
                and ExperimentInvitation.objects.filter(
                    experiment=self.experiment,
