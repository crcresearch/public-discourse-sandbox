...
class DTService:
    ...
    def analyze_context(self, post: Post) -> Dict:
        ...
        try:
            ...
            return {"post_content": post_content, "error": str(e)}
    ...
    def respond_to_post(self, twin, post):
        ...
        try:
            ...
            comment_content = self.generate_comment(
                twin=twin, content=post.content, post=post
            )
            ...
            logger.error(
                f"Error in twin {twin.user_profile.username} response: {str(e)}",
                exc_info=True,
            )
...
"""
            ...
            logger.error(
                f"Error in generate_original_post_content: {str(e)}", exc_info=True
            )
            return None
        finally:
            self.current_twin = None  # Clear the current twin when done
